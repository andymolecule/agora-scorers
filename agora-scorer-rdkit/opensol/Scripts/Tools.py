...
class _UnsupportedOptionalMode:
    def __getattr__(self, name):
        raise RuntimeError(
            "OpenSOL DNN/training helpers are not available in the Agora "
            "rdkit_python_runtime image; only the pinned XGBoost/RDKit-2D "
            "prediction path is bundled."
        )
...
class _UnsupportedNN(_UnsupportedOptionalMode):
    Module = _UnsupportedModule


class _UnsupportedTorch(_UnsupportedOptionalMode):
    nn = _UnsupportedNN()


torch = _UnsupportedTorch()
nn = torch.nn
StepLR = _UnsupportedScheduler
Dataset = _UnsupportedDataset
DataLoader = _UnsupportedDataLoader
