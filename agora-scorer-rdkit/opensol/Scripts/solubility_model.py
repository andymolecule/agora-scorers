...
class Model4logS():
    ...
    def logS_calc(self, file_in, file_out, clogp):
        ...
        if self.params['model'] == 'random_forest':
            ...
            expected_feature_count = model.num_features()
            if df_dscr.shape[1] > expected_feature_count:
                df_dscr = df_dscr.iloc[:, :expected_feature_count]
            elif df_dscr.shape[1] < expected_feature_count:
                sys.exit(
                    "OpenSOL RDKit descriptor count is lower than the pinned "
                    "XGBoost model expects: {} < {}".format(
                        df_dscr.shape[1], expected_feature_count
                    )
                )
